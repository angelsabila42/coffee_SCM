...
.chat-footer {
    ...
    align-items: center;
    gap: 1rem;
    box-shadow: 0 -2px 8px rgba(139,94,60,0.03);
}
...
.chat-footer .send-btn {
    ...
    font-size: 1.3rem;
    transition: background 0.2s, box-shadow 0.2s;
    box-shadow: 0 2px 8px rgba(139,94,60,0.10);
    margin-left: 0.5rem;
    margin-right: 0.2rem;
    position: relative;
    z-index: 2;
        ...
        @if(isset($conversation))
            ...
            <div class="chat-footer">
                <form action="{{ route('chat.store', $conversation->id) }}" method="POST" id="messageForm" class="d-flex w-100 align-items-center gap-2">
                    @csrf
                    <input type="text" name="message" class="form-control" placeholder="Write something..." id="messageInput" required autocomplete="off" style="background:#fff;">
